...
* Use pip freeze to generate the requirements.txt:
...
Be sure to use the Postgres Dockerfile from last lesson, but just rename it.
The Dockerfile.api should be the Dockerfile explained above.
...
#### Step 2: Build the PostgreSQL Docker Image

Build the PostgreSQL Docker image using the same commands as from last lesson using the command below:

```bash
docker build -t my-django-postgres-container -f ./Dockerfile.postgres .
```

No explanation is given since this is from the last lesson.
...
* Before running the Django API container, ensure only the PostgreSQL container is running. If the API container is running, stop it and remove it first:
