...
Before you can use Docker, you must install it and ensure that it works for the operating system that you are using. For this we will utilize **Docker Desktop**. Docker Desktop is a GUI that makes using Docker much easier than with a command line alone. With Docker Desktop we can view all the containers that are currently running and modify them, as well as many other actions that will be explored later.

### Step 1: Download Docker Desktop 

For non-Windows users, download Docker desktop using one of the links below based on your operating system. 
For Windows users, you must follow the instructions in the WSL2 guide before downloading directly, since we use WSL2.

* [Docker Desktop Mac link](https://docs.docker.com/desktop/install/mac-install/)
* [Docker Desktop Linux link](https://docs.docker.com/desktop/install/linux-install/)
* [Docker Desktop Windows WSL instructions link](https://docs.docker.com/desktop/wsl/)

#### Explanation

* We need to download Docker Desktop so that we can use Docker. 
* Without Docker Desktop we have to manually install it using the command line. 

### Step 2: Restart and ensure it works

After installing Docker desktop, you will be asked to log out or restart your computer, do that to proceed.
After restarting your computer, open your terminal and ensure docker works by trying out docker in the terminal. You should see no errors.
...
* This step is primarily to ensure that Docker was installed correctly. 
* If it is installed correctly, when you do the docker command, it should be recognized as a valid command in the cli. 
* You should not see any errors related to not having the command docker. 
* If you do see errors, it indicates you didn't install it correctly.

### Step 3: Add user to the docker group

Finally, after verifying that it is downloaded, you must add your user to the docker group by doing the following commands.
```bash
sudo usermod -aG docker $USER
```

Close your terminal and re-open it again. Then type **"groups"** to verify that docker is one of the groups.
```bash
groups
>> $USER adm dialout cdrom floppy sudo audio dip video plugdev netdev docker
```

#### Explanation

* Adding your user to the docker group is essential since you can not build your Docker Image if your user does not have permission to do so. 
* If you do not add your user to the docker group, then you will see an error saying you do not have permissions.
